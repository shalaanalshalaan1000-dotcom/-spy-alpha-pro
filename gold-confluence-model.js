...
function fibonacciLocation(bars,price){
  ...
  const pos=(price-low)/range;let buy=0,sell=0,location='MID';if(pos<=.382){buy=6;location='DEEP_DISCOUNT';}else if(pos<=.50){buy=4;location='DISCOUNT';}else if(pos<=.618){buy=2;location='LOW_MID';}if(pos>=.618){sell=6;location='DEEP_PREMIUM';}else if(pos>=.50){sell=4;location='PREMIUM';}else if(pos>=.382){sell=2;location='HIGH_MID';}
...
export function analyzeGoldSignal(samples,rawPrice,now=Date.now()){
  ...
  const minMove=Math.max(1.25,atr1*1.2);const fallbackTargets=nearestTargets(side,entry,bars,minMove,risk);for(let i=0;i<4;i++)if(targets[i]==null||(side==='BUY'?targets[i]<=entry:targets[i]>=entry))targets[i]=fallbackTargets[i];for(let i=1;i<4;i++){if(side==='BUY'&&targets[i]<=targets[i-1]+.35)targets[i]=targets[i-1]+Math.max(.75,minMove*.45);if(side==='SELL'&&targets[i]>=targets[i-1]-.35)targets[i]=targets[i-1]-Math.max(.75,minMove*.45);}labels=[0,1,2,3].map((i)=>labels[i]||`CONFLUENCE_TP${i+1}`);
