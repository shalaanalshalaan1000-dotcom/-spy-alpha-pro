...
const replacements = [
  [
    ...
    "import { analyzeGoldSignal } from './gold-momentum-model.js';\nimport { getGoldNewsRisk } from './gold-news-risk.js';"
  ],
  [
...
// and size risk by lot instead of forcing a tight dollar stop.
{
  ...
  if(!source.includes(oldNormalize))throw new Error('ICT patch: normalize anchor missing');
  ...
  if(!source.includes(oldAdd))throw new Error('ICT patch: add anchor missing');
  ...
  if(stopStart<0||stopEnd<0)throw new Error('ICT patch: structuralStop anchor missing');
  ...
  if(dayAt<0)throw new Error('ICT patch: day formatter anchor missing');
  ...
  if(!source.includes(oldManage))throw new Error('ICT patch: management anchor missing');
