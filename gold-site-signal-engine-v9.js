...
// Murphy upgrade: preserve HTF history/OHLC, use structural stops, support/resistance targets,
// and size risk by lot instead of forcing a tight dollar stop.
{
  ...
  if(!source.includes(oldNormalize))throw new Error('Murphy patch: normalize anchor missing');
  ...
  if(!source.includes(oldAdd))throw new Error('Murphy patch: add anchor missing');
  ...
  if(validStart<0||validEnd<0)throw new Error('Murphy patch: validLevels anchor missing');
  ...
  if(!source.includes(m5Anchor))throw new Error('Murphy patch: config anchor missing');
  ...
  if(dayAt<0)throw new Error('Murphy patch: day formatter anchor missing');
  ...
  if(!source.includes(oldManage))throw new Error('Murphy patch: management anchor missing');
