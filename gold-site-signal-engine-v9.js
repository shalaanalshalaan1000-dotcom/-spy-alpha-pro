...
const replacements = [
  ...
  [
    ...
    "const BUILD='site-signal-noai-v35-ict-relaxed-observable';"
  ],
  [
  ...
  [
    ...
    "source:'GOLD_ALPHA_SITE',executionMode:'SIGNAL_ONLY_TELEGRAM',executable:false,entered:true,triggered:true,triggerPrice:p,priceProvider:q.provider,lockedTargets:true,tradeStyle:'ICT_TOP_DOWN_SWING',maxDailySignals:MAX_DAILY_SIGNALS,dailySignalNumber:state.dailySignalCount+1,newsRiskAtEntry:state.lastNewsRisk,"
  ],
  [
  ...
  [
    ...
    "signalConfidence:Number(state.signal?.confidence??model.confidence??0),minConfidence:MIN_CONFIDENCE,dailySignalCount:state.dailySignalCount,maxDailySignals:MAX_DAILY_SIGNALS,tradeStyle:'ICT_TOP_DOWN_SWING',newsRisk,volatilityPolicy:policy,"
  ],
  [
...
// and size risk by lot instead of forcing a tight dollar stop.
{
  ...
  const stopFn="function structuralStop(side,entry,modelStop,now=Date.now()){const policy=volatilityPolicy(now),stop=n(modelStop);if(stop==null||!Number.isFinite(entry))return{ok:false,reason:'INVALID_ICT_STOP',policy};if(side==='BUY'&&stop>=entry)return{ok:false,reason:'INVALID_ICT_STOP_SIDE',policy};if(side==='SELL'&&stop<=entry)return{ok:false,reason:'INVALID_ICT_STOP_SIDE',policy};const risk=Math.abs(entry-stop);if(risk<.45)return{ok:false,reason:'ICT_STOP_TOO_TIGHT',stop:round(stop,3),risk:round(risk,3),policy};return{ok:true,stop:round(stop,3),risk:round(risk,3),swing:null,buffer:0,minDistance:0,policy,source:'ICT_MODEL'};}";
  ...
  source=source.replace("reason:\`CONFIRMED GOLD SETUP — \${Number(m.confidence)||0}% confidence; 5m structure stop + \${round(stopPlan.buffer,2)}$ buffer; 5m execution + 15m context with 1m used only for timing; USD news guard clear; \${round(viability.rr,2)}R to TP1, risk \${round(viability.risk,2)}$, ATR1 \${viability.policy.atr1}$; max \${MAX_DAILY_SIGNALS}/day; Telegram only, AI off\`","reason:\`ICT CONFIRMED — \${m.strategy||'ICT'}; HTF 4H/1H + 15m context + 5m execution; draw on \${m.ict?.drawOnLiquidity||m.targetLabels?.[0]||'opposing liquidity'}; live RR \${round(liquidityRR,2)}R; SL \${round(sl,2)}; lot \${lotSizing.recommendedLot||0}; USD news guard clear\`");
