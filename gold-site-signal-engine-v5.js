...
const BUILD='site-signal-noai-v16-tradingview-lp';
const TV_SYMBOL=String(process.env.TV_PUBLIC_SYMBOL||'OANDA:XAUUSD').trim();
const MAX_QUOTE_AGE_MS=20_000;
const state={samples:[],signal:null,lastTerminal:null,trades:[],cooldownUntil:0,quote:null,lastError:null,ws:null,wsConnected:false,lastTvAt:0,lastLpAt:0,loggedQuote:false,loggedLp:false};
...
server.listen(PORT,'0.0.0.0',()=>{console.log(`[gold-site-signal-engine] ${BUILD} listening on ${PORT}; TradingView OANDA LP primary + 1m chart fallback; immutable trade levels; no subscription; AI=off; execution=off; telegram-only`);connectTradingView();});
