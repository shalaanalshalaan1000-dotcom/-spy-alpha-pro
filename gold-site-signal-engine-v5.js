...
const BUILD='site-signal-noai-v6-yahoo-goldprice';
...
async function getJson(url,timeout=6000){const r=await fetch(url,{cache:'no-store',headers:{accept:'application/json','user-agent':'Mozilla/5.0 GoldAlpha/6.0'},signal:AbortSignal.timeout(timeout)});const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(String(d?.error||d?.detail||`HTTP_${r.status}`));return d;}
...
async function goldPriceDevQuote(){
 const now=Date.now();
 const d=await getJson('https://goldprice.dev/api/v1/prices?symbol=XAU-USD-SPOT',6000);
...
async function xausQuote(){const now=Date.now(),d=await getJson(`https://xaus.com/api/v1/spot?fresh=${now}`,5000);const price=n(d.spot_usd_oz??d?.xau?.price),t=Date.parse(d.price_as_of||d.updated_at||'');if(price==null||price<=0)throw new Error('XAUS_BAD_PRICE');const stamp=Number.isFinite(t)?t:now,age=Math.max(0,now-stamp);if(d.stale===true||age>MAX_QUOTE_AGE_MS)throw new Error(`XAUS_STALE_${Math.round(age/1000)}s`);return{price,bid:price,ask:price,t:stamp,provider:'XAUS',degraded:false};}
async function fetchQuote(){
 ...
 let err;for(const fn of [yahooQuote,goldPriceDevQuote,xausQuote]){try{const q=await fn();state.quote=q;state.quoteAt=now;state.lastError=null;return q}catch(e){err=e;}}
