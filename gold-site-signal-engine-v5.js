...
const MIN_LIVE_TP1_R=Math.max(.5,Number(process.env.GOLD_MIN_LIVE_TP1_R||.90));
const MIN_LIVE_TP1_USD=Math.max(.25,Number(process.env.GOLD_MIN_LIVE_TP1_USD||.75));
const BUILD='site-signal-noai-v17-entryability-guard';
const TV_SYMBOL=String(process.env.TV_PUBLIC_SYMBOL||'OANDA:XAUUSD').trim();
const MAX_QUOTE_AGE_MS=20_000;
const state={samples:[],signal:null,lastTerminal:null,trades:[],cooldownUntil:0,quote:null,lastError:null,ws:null,wsConnected:false,lastTvAt:0,lastLpAt:0,loggedQuote:false,loggedLp:false,lastEntryGuard:null};
...
function maybeCreate(m,q,now){
 ...
 if(!viability.ok){
  state.lastEntryGuard={atMs:now,reason:viability.reason,side,price:round(p,3),risk:round(viability.risk,3),rewardToTp1:round(viability.reward,3),liveTp1R:round(viability.rr,2),minTp1R:MIN_LIVE_TP1_R,minTp1Usd:MIN_LIVE_TP1_USD};
  console.warn(`[entry-guard] rejected ${side} at ${round(p,3)}: ${viability.reason}; TP1 room ${round(viability.reward,3)} / ${round(viability.rr,2)}R`);
  return;
 }
 state.signal={signalId:`XAU-${now}-${side}`,side,candidateAction:side,action:'WAIT',status:'ACTIVE',strategy:m.strategy||null,confidence:Number(m.confidence)||0,signalConfidence:Number(m.confidence)||0,entry:n(m.entry),entryLow:lo,entryHigh:hi,stopLoss:sl,target1:tp1,target2:n(m.target2),target3:n(m.target3),target4:n(m.target4),riskReward:n(m.riskReward),liveTp1R:round(viability.rr,2),tp1RoomUsd:round(viability.reward,3),prediction:m.prediction||null,contextBias:m.contextBias||null,oneMinuteConfirmed:Boolean(m.oneMinuteConfirmed),issuedAt:iso(now),issuedAtMs:now,targetHits:[false,false,false,false],targetHitAt:[null,null,null,null],source:'GOLD_ALPHA_SITE',executionMode:'SIGNAL_ONLY_TELEGRAM',executable:false,entered:true,triggered:true,triggerPrice:p,priceProvider:q.provider,quoteAgeSec:round(quoteAgeMs(q,now)/1000,1),lockedLevels:true,reason:`CONFIRMED BY SITE ENGINE ON FRESH ${q.provider} PRICE — entryability guard passed (${round(viability.rr,2)}R to TP1); Telegram only, AI off`};state.trades.push({...state.signal,status:'SIGNAL'});state.trades=state.trades.slice(-200);
}
async function evaluate(){const q=state.quote;const now=Date.now();if(!freshQuote(q,now))throw new Error(`TRADINGVIEW_DIRECT_NOT_FRESH age=${Number.isFinite(quoteAgeMs(q,now))?round(quoteAgeMs(q,now)/1000,1):'na'}s connected=${state.wsConnected}`);manage(q,now);const model=analyzeGoldSignal(state.samples,q.price,now);if(!state.signal)maybeCreate(model,q,now);const age=quoteAgeMs(q,now);const base={...model,source:'GOLD_ALPHA_SITE',executionMode:'SIGNAL_ONLY_TELEGRAM',executable:false,provider:q.provider,degraded:false,liveFeedFresh:true,quoteAgeMs:Math.round(age),quoteAgeSec:round(age/1000,1),maxQuoteAgeSec:MAX_QUOTE_AGE_MS/1000,price:q.price,bid:q.bid,ask:q.ask,updatedAt:iso(now),sampleCount:Number(model.sampleCount||state.samples.length),signalConfidence:Number(state.signal?.confidence??model.confidence??0),minConfidence:MIN_CONFIDENCE,minLiveTp1R:MIN_LIVE_TP1_R,minLiveTp1Usd:MIN_LIVE_TP1_USD,entryGuard:state.lastEntryGuard,terminalEvent:state.lastTerminal,historySource:'TRADINGVIEW_OANDA_1M',build:BUILD,noAI:true,subscriptionRequired:false,tvSymbol:TV_SYMBOL,livePriceMode:q.provider};if(state.signal)return{...base,...state.signal,status:'ACTIVE',action:'WAIT',candidateAction:state.signal.side,tp1:state.signal.targetHits[0],tp2:state.signal.targetHits[1],tp3:state.signal.targetHits[2],tp4:state.signal.targetHits[3],price:q.price,bid:q.bid,ask:q.ask,updatedAt:iso(now),terminalEvent:state.lastTerminal,historySource:'TRADINGVIEW_OANDA_1M',build:BUILD,noAI:true,subscriptionRequired:false,tvSymbol:TV_SYMBOL,livePriceMode:q.provider,reason:state.signal.reason};if(state.lastEntryGuard?.atMs===now)return{...base,status:'WAIT',action:'WAIT',candidateAction:'WAIT',reason:`LATE_ENTRY_GUARD: ${state.lastEntryGuard.reason}; TP1 room ${state.lastEntryGuard.rewardToTp1}$ / ${state.lastEntryGuard.liveTp1R}R — no Telegram entry sent`};if(now<state.cooldownUntil)return{...base,status:'WAIT',action:'WAIT',candidateAction:'WAIT',reason:'COOLDOWN: waiting after previous trade'};return{...base,action:'WAIT',reason:model.status==='CANDIDATE'?'السيناريو جاهز وينتظر لمس نطاق الدخول على سعر TradingView OANDA الحي':(model.reason||'بانتظار إشارة مؤكدة من محرك الموقع')};}
...
server.listen(PORT,'0.0.0.0',()=>{console.log(`[gold-site-signal-engine] ${BUILD} listening on ${PORT}; TradingView OANDA LP primary + 1m chart fallback; immutable trade levels; entryability guard ${MIN_LIVE_TP1_R.toFixed(2)}R/${MIN_LIVE_TP1_USD.toFixed(2)}$; no subscription; AI=off; execution=off; telegram-only`);connectTradingView();});
