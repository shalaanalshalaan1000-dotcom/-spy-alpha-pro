...
const BUILD='site-signal-noai-v8-goldprice-live';
...
async function getJson(url,timeout=6000){const r=await fetch(url,{cache:'no-store',headers:{accept:'application/json','user-agent':'Mozilla/5.0 GoldAlpha/8.0'},signal:AbortSignal.timeout(timeout)});const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(String(d?.error||d?.detail||`HTTP_${r.status}`));return d;}
...
async function fetchQuote(){
 const now=Date.now();
 if(state.quote&&now-state.quoteAt<40_000&&now-state.quote.t<=MAX_QUOTE_AGE_MS)return state.quote;
