...
function applyBtcAutoUi(source) {
  const css = `
.btcAutoPanel{margin:0 0 14px;border:1px solid #8a5a24;border-radius:18px;background:linear-gradient(145deg,#21170c,#0b111b);padding:16px}.btcAutoHead{display:flex;justify-content:space-between;gap:12px;align-items:flex-start;margin-bottom:12px}.btcAutoHead h2{margin:0;color:#f3bf66;font-size:18px}.btcAutoHead p{margin:5px 0 0;color:#b7a07a;font-size:11px}.btcAutoBadge{padding:6px 9px;border:1px solid #9d6d31;border-radius:999px;color:#f3bf66;background:#2a1c0d;font-size:11px;font-weight:900}.btcAutoGrid{display:grid;grid-template-columns:repeat(4,1fr);gap:9px}.btcAutoCard{padding:11px;border:1px solid #5f4325;border-radius:11px;background:#0d1421}.btcAutoCard span,.btcAutoCard small{display:block;color:#ad9979;font-size:11px}.btcAutoCard strong{display:block;margin:6px 0;font-size:18px;direction:ltr;text-align:right}.btcAutoNote{margin:10px 0 0;color:#ad9979;font-size:11px;line-height:1.7}@media(max-width:760px){.btcAutoGrid{grid-template-columns:repeat(2,1fr)}.btcAutoHead{flex-direction:column}}
`;
  source = replaceRequired(source, '</style></head>', css + '</style></head>', 'style end');

  const panel = `<article class="btcAutoPanel"><div class="btcAutoHead"><div><h2>BTCUSD — التداول الآلي السريع</h2><p>محرك سكالب مستقل للبيتكوين • لا يتأثر بإغلاق الذهب أو تجميع شموعه</p></div><span id="btcAutoBadge" class="btcAutoBadge">BTC MT5 READY</span></div><section class="btcAutoGrid"><div class="btcAutoCard"><span>إشارة البيتكوين</span><strong id="btcAutoAction">انتظار</strong><small id="btcAutoConfidence">فحص السوق</small></div><div class="btcAutoCard"><span>نطاق الدخول</span><strong id="btcAutoRange">—</strong><small>نافذة تنفيذ قصيرة</small></div><div class="btcAutoCard"><span>وقف الخسارة</span><strong id="btcAutoStop">—</strong><small>مستقل عن وقف الذهب</small></div><div class="btcAutoCard"><span>هدف السكالب الأول</span><strong id="btcAutoTarget">—</strong><small>هدف قريب لخروج سريع</small></div></section><p id="btcAutoNote" class="btcAutoNote">يبحث محرك BTC المستقل عن اندفاع أو انعكاس واضح قبل إرسال الصفقة إلى MT5.</p></article>`;
...
async function loadBtcAutoTrade(){if(btcAutoTradeLoading)return;btcAutoTradeLoading=true;try{const r=await fetch('/api/auto-trade/signal?asset=BTCUSD&observe=1',{cache:'no-store'}),d=await r.json();if(!r.ok||d.error)throw new Error(d.error||'BTC signal request failed');const candidate=d.action!=='WAIT'?d.action:d.candidateAction,active=['BUY','SELL'].includes(candidate),label=candidate==='BUY'?'شراء':candidate==='SELL'?'بيع':'انتظار',money=v=>v!=null&&Number.isFinite(Number(v))&&Number(v)>0?'$'+Number(v).toFixed(2):'—';$('#btcAutoAction').textContent=label;$('#btcAutoAction').className=candidate==='BUY'?'positive':candidate==='SELL'?'negative':'WATCH';$('#btcAutoConfidence').textContent='الثقة '+Number(d.confidence||0)+'% • '+String(d.strategy||'BTC SCALP');$('#btcAutoRange').textContent=active?money(d.entryLow)+' — '+money(d.entryHigh):'—';$('#btcAutoStop').textContent=active?money(d.stopLoss):'—';$('#btcAutoTarget').textContent=active?money(d.target1):'—';$('#btcAutoBadge').textContent=d.status==='ACTIVE'?'إشارة BTC فعالة':d.status==='STALE'?'بيانات قديمة':'BTC MT5 READY';$('#btcAutoNote').textContent=d.reason||'بانتظار فرصة سكالب مكتملة للبيتكوين';}catch(error){$('#btcAutoBadge').textContent='غير متاح';$('#btcAutoNote').textContent=error.message}finally{btcAutoTradeLoading=false}}
`;
  ...
  for (const marker of ['id="btcAutoAction"', 'asset=BTCUSD&observe=1', 'loadBtcAutoTrade()', 'setInterval(loadBtcAutoTrade,5000)']) {
