...
`;
  ...
  if(source.includes(serverAnchor)&&!source.includes("/api/performance/journal")){
    source=source.replace(serverAnchor,serverAnchor+"const perfUrl=new URL(req.url||'/',`http://${req.headers.host||'localhost'}`);if(req.method==='GET'&&perfUrl.pathname==='/api/performance/journal'){const rows=readCommercialJournal();return json(res,200,{ok:true,persistent:!String(GOLD_ALPHA_JOURNAL_FILE).startsWith('/tmp/'),metrics:commercialMetrics(rows),trades:rows.slice().reverse().slice(0,100)})}");
  }
  ...
  if(!source.includes('function refreshGoldPerformance')){
    if(source.includes('(async()=>{'))source=source.replace('(async()=>{',client+'\n(async()=>{');
    else if(source.includes('</script></body>'))source=source.replace('</script></body>',client+'</script></body>');
  }
  return source;
}
