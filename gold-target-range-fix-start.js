...
function patchTargetTracking(source){
  ...
  // Keep `r` in function scope because the generated UI code may reference it after this
  // expression. Prefer the auto engine quote; fall back to Gold API for display only.
  const resilientQuote="let r,raw;try{r=await fetch('/api/auto-trade/signal?observe=1&_='+Date.now(),{cache:'no-store'});raw=await r.json();if(!r.ok||!Number.isFinite(Number(raw?.price))||!raw?.updatedAt||raw?.degraded===true)throw new Error(raw?.reason||'engine quote unavailable')}catch(_e){const fr=await fetch('https://api.gold-api.com/price/XAU',{cache:'no-store'}),fd=await fr.json();if(!fr.ok||!Number.isFinite(Number(fd?.price))||Number(fd.price)<=0)throw new Error('تعذر تحميل سعر الذهب');r=fr;raw={...fd,updatedAt:fd.updatedAt||new Date().toISOString(),provider:'GOLD_API_FALLBACK',degraded:true}}const d=goldBrowserReading(raw);";
