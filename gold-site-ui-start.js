...
function patchSiteSignalUi(source) {
  ...
  const scenarioLabel=active?(side==='BUY'?'BUY مؤكد — ICT Fast':'SELL مؤكد — ICT Fast'):candidate?('مرشح '+candidateSide+' — ICT Fast'):recovering?'استعادة البيانات':cooldown?'انتظار بعد الصفقة':(['BUY','SELL'].includes(raw?.contextBias)?('ICT Context '+raw.contextBias):'انتظار');
  ...
  source = source.replace(
    ...
    'XAUUSD — ICT FAST SCALP | 15m Context + 1m/5m Trigger'
  );
  source = source.replace(
  ...
  source = source.replace(
    ...
    'ICT سريع: يحدد اتجاه 15m ثم يبحث عن FVG + MSS/Displacement على 1m/5m، مع أهداف قريبة متدرجة.'
  );
...
(function(){
 ...
 function missing(raw,conf,min){
  ...
  if(st==='CANDIDATE')return 'ICT Fast مرشح؛ الخادم يتحقق من سعر الدخول والمخاطرة';
