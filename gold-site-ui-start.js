...
function patchSiteSignalUi(source) {
  ...
  source = source.replace(
    ...
    "$('#goldConfidence').textContent=(plan.state==='UP'||plan.state==='DOWN')?'سياق ICT مكتمل':(plan.candidate?(plan.ict?.entryMode==='CONFIRMED_CONTINUATION'?'Continuation confirmed — دخول مسلح':'Origin FVG محددة — انتظار retracement'):(plan.state==='COLLECTING'?'تهيئة بيانات ICT':'بانتظار اكتمال سياق ICT'));"
  );
  source = source.replace(
 ...
 async function refresh(){
  try{
   ...
   set('goldSetupProgress',progress+'%');set('goldSetupSteps',checks.map(x=>x[0]+' '+(x[1]?'✓':'—')).join(' • '));set('goldMissingCondition',missing(raw,conf,min));
    set('goldConfidence',active?'سياق ICT مكتمل':(status==='CANDIDATE'?(continuationReady?'Continuation confirmed — دخول مسلح':'Origin FVG محددة — انتظار retracement'):'بانتظار اكتمال سياق ICT'));
