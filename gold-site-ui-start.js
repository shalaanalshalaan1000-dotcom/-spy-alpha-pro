...
function patchSiteSignalUi(source) {
  ...
  const momentum=raw?.murphy||raw?.momentum||raw?.prediction||{},trend=momentum?.trend||{},trigger=momentum?.trigger||{},rsi5=Number(momentum?.rsi5??raw?.prediction?.rsi5),structure5=Number(momentum?.structure5m??0);
  ...
  source = source.replace(
    ...
    'John J. Murphy XAUUSD — Trend + Support/Resistance + Momentum'
  );
  source = source.replace(
...
(function(){
 ...
 function missing(raw,conf,min){
  ...
  if(/MURPHY WAIT/.test(u))return s.replace(/^MURPHY WAIT\\s*[—:-]?\\s*/i,'');
  if(st==='CANDIDATE')return 'مرشح Murphy وينتظر دخول منطقة التنفيذ';
  return 'بانتظار توافق اتجاه 60m و15m ثم إغلاق 5m مؤكد';
 }
 async function refresh(){
  try{
   ...
   const mom=raw?.murphy||raw?.momentum||raw?.prediction||{},trend=mom?.trend||{},trigger=mom?.trigger||{},rsi5=Number(mom?.rsi5??raw?.prediction?.rsi5),structure5=Number(mom?.structure5m??0);
   const trendReady=['BUY','SELL'].includes(raw?.contextBias),confirm15=side==='BUY'?Boolean(trend?.bull15):side==='SELL'?Boolean(trend?.bear15):Boolean(trend?.bull15||trend?.bear15),triggerReady=active||status==='CANDIDATE'||Boolean(trigger?.ready),rsiReady=!Number.isFinite(rsi5)?status==='CANDIDATE':(side==='BUY'?(rsi5>=50&&rsi5<=76):side==='SELL'?(rsi5<=50&&rsi5>=24):false);
   const checks=[['60m Trend',trendReady],['15m Confirm',confirm15],['5m Trigger',triggerReady],['RSI / Entry',rsiReady]],done=checks.filter(x=>x[1]).length;
   ...
   set('goldIctDraw','RSI5 '+(Number.isFinite(rsi5)?rsi5.toFixed(1):'—')+' • '+String(trigger?.type||'—'));set('goldIctBias','60m/15m: '+String(trend?.side||raw?.contextBias||'NEUTRAL')+' • 5m structure '+(Number.isFinite(structure5)?structure5:'—'));
