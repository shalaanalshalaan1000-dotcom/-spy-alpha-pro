...
function patchSiteSignalUi(source) {
  ...
  const trendReady=['BUY','SELL'].includes(raw?.contextBias),sidePlan=candidateSide==='BUY'?scenarioPlan?.buy:candidateSide==='SELL'?scenarioPlan?.sell:null,confirm15=candidateSide?Boolean(sidePlan?.alignedWithBias):trendReady,triggerReady=active||candidate||Boolean(trigger?.ready),rsiReady=!Number.isFinite(rsi5)?candidate:(candidateSide==='BUY'?(rsi5>=50&&rsi5<=76):candidateSide==='SELL'?(rsi5<=50&&rsi5>=24):false);
  const checks=[['60m Trend',trendReady],['15m Context',confirm15],['5m Trigger',triggerReady],['RSI / Entry',rsiReady]],done=checks.filter(x=>x[1]).length,setupProgress=active?100:collecting?Math.min(15,Math.round((Number(raw?.sampleCount)||0)/5000*15)):Math.round(done/checks.length*100),setupSteps=checks.map(x=>x[0]+' '+(x[1]?'✓':'—')).join(' • ');
  const reasonUpper=reason.toUpperCase();let missingCondition='بانتظار وصول السعر للمنطقة ثم إغلاق 5m مؤكد؛ 15m عامل تعزيز وليس شرط منع';
