...
function patchSiteSignalUi(source) {
  ...
  source = source.replace(
    ...
    "$('#goldConfidence').textContent=(plan.state==='UP'||plan.state==='DOWN')?'سياق ICT مكتمل':(plan.candidate?'Origin FVG محددة — انتظار retracement':(plan.state==='COLLECTING'?'تهيئة بيانات ICT':'بانتظار اكتمال سياق ICT'));"
  );
  source = source.replace(
...
(function(){
 ...
 function missing(raw,conf,min){
  ...
  if(st==='CANDIDATE')return 'Origin FVG محددة؛ ننتظر رجوع السعر إلى منطقة الدخول';
 ...
 async function refresh(){
  try{
   ...
   const ict=raw?.ict||{},sweepEvent=ict?.legSweep||ict?.sweep||null,sweepLabel=String(sweepEvent?.name||'Liquidity').replaceAll('_',' '),contextReady=Number(ict?.dir15)!==0||Number(ict?.dir1)!==0,sweepReady=Boolean(sweepEvent),mssReady=sweepReady&&Boolean(ict?.hasShift||ict?.mss||ict?.firstMssEvent),displacementReady=sweepReady&&Boolean(ict?.hasDisplacement||ict?.displacement||ict?.firstDisplacementEvent),shiftReady=mssReady&&displacementReady,fvgReady=shiftReady&&Boolean(ict?.originFvg),targetReady=fvgReady&&Boolean(ict?.mainLiquidity||raw?.target1),triggerReady=active;
   const checks=[['Context',contextReady],[sweepReady?sweepLabel+' swept':'Sweep',sweepReady],['MSS + Displacement',shiftReady],['Origin FVG',fvgReady],['External Liquidity',targetReady],['Entry',triggerReady]],done=checks.filter(x=>x[1]).length;
   ...
   set('goldConfidence',active?'سياق ICT مكتمل':(status==='CANDIDATE'?'Origin FVG محددة — انتظار retracement':'بانتظار اكتمال سياق ICT'));
   ...
   set('goldIctDraw',ict?.drawOnLiquidity||raw?.targetLabels?.[0]||'External liquidity targets');set('goldIctBias',(side?('Execution '+side+' • '):'')+'15m '+(ict?.dir15===1?'↑':ict?.dir15===-1?'↓':'—')+' • 1H '+(ict?.dir1===1?'↑':ict?.dir1===-1?'↓':'—')+' • '+String(ict?.session||'—'));
