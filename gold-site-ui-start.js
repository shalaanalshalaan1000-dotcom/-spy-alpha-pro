...
function patchSiteSignalUi(source) {
  ...
  const biasReady=Boolean(trendSign&&(ict.dir1===trendSign||ict.dir1===0)),setupReady=Boolean(ict.setupReady||(trendSign&&ict.dir15===trendSign)),fvgReady=Boolean(ict.fvg5||ict.fvg),impulseReady=Boolean(ict.dm5?.displacement||ict.dm5?.mss||ict.displacement||ict.mss),rrValue=Number(raw?.riskReward),rrReady=Number.isFinite(rrValue)&&rrValue>=1.5;
  const checks=[['1H Bias',biasReady],['15m Setup',setupReady],['5m FVG',fvgReady],['5m Impulse',impulseReady],['RR≥1.5',rrReady]],done=checks.filter(x=>x[1]).length,setupProgress=active?100:collecting?Math.min(15,Math.round((Number(raw?.sampleCount)||0)/120*15)):Math.round(done/checks.length*100),setupSteps=checks.map(x=>x[0]+' '+(x[1]?'✓':'—')).join(' • ');
  const reasonUpper=String(raw?.reason||'').toUpperCase();let missingCondition='بانتظار اكتمال شروط ICT';
  if(recovering)missingCondition='استعادة بيانات السعر الحي';else if(cooldown)missingCondition='فترة حماية بعد الصفقة السابقة';else if(/1H BIAS AND 15M SETUP ARE NOT ALIGNED/.test(reasonUpper))missingCondition='ينقص توافق 15m مع اتجاه 1H';else if(/15M SETUP NEEDS TWO/.test(reasonUpper))missingCondition='15m يحتاج اثنين من: Sweep / FVG / Displacement-MSS';else if(/WAITING FOR 5M FVG/.test(reasonUpper))missingCondition='إعداد 15m جاهز؛ ننتظر دخول 5m';else if(/STRUCTURAL INVALIDATION IS TOO CLOSE/.test(reasonUpper))missingCondition='مسافة وقف الهيكل غير مناسبة';else if(/NO OPPOSING LIQUIDITY TARGET/.test(reasonUpper))missingCondition='لا يوجد هدف سيولة يحقق 1.5R';else if(/WAITING_NEXT_5M_CLOSE_WINDOW/.test(reasonUpper))missingCondition='انتظار نافذة إغلاق 5m';else if(/WAITING_1M_CONFIRMATION/.test(reasonUpper))missingCondition='ينقص تأكيد 1m';else if(/USD_NEWS_BLACKOUT|NEWS RISK/.test(reasonUpper))missingCondition='فلتر الأخبار يمنع الدخول مؤقتًا';else if(raw?.status==='CANDIDATE')missingCondition='الإعداد جاهز مبدئيًا وينتظر دخول السعر/فلاتر التنفيذ';
...
(function(){
 ...
 function missing(raw){
  ...
  if(/1H BIAS AND 15M SETUP ARE NOT ALIGNED/.test(s))return 'ينقص توافق 15m مع اتجاه 1H';
  if(/15M SETUP NEEDS TWO/.test(s))return '15m يحتاج اثنين من: Sweep / FVG / Displacement-MSS';
  ...
  return raw?.contextBias&&raw.contextBias!=='NEUTRAL'?'ينتظر اكتمال إعداد 15m/5m':'بانتظار Bias واضح من 1H أو 15m';
 }
 async function refresh(){
  try{
   ...
   const checks=[['1H Bias',Boolean(sign&&(ict.dir1===sign||ict.dir1===0))],['15m Setup',Boolean(ict.setupReady||(sign&&ict.dir15===sign))],['5m FVG',Boolean(ict.fvg5||ict.fvg)],['5m Impulse',Boolean(ict.dm5?.displacement||ict.dm5?.mss||ict.displacement||ict.mss)],['RR≥1.5',Number.isFinite(Number(raw?.riskReward))&&Number(raw.riskReward)>=1.5]];
