...
function patchSiteSignalUi(source) {
  ...
  const ict=raw?.ict||{},cf=raw?.confluence||{},scores=cf?.scores||{},breakdown=cf?.breakdown||{},momentum=cf?.momentum||{},scenarioPlan=null,rsi5=Number(cf?.momentum?.rsi14),structure5=cf?.structure?.m5==='BUY'?1:cf?.structure?.m5==='SELL'?-1:0;
  ...
  const componentRows=[['Structure',Number(breakdown?.structure?.[selected])||0,24],['Trend',Number(breakdown?.trend?.[selected])||0,18],['Momentum',Number(breakdown?.momentum?.[selected])||0,14],['Price Action',Number(breakdown?.priceAction?.[selected])||0,16],['Liquidity',Number(breakdown?.liquidity?.[selected])||0,12],['Fib',Number(breakdown?.location?.[selected])||0,6],['Volatility',Number(breakdown?.volatility?.[selected])||0,10]];
  const setupProgress=active?100:collecting?Math.min(15,Math.round((Number(raw?.sampleCount)||0)/5000*15)):confidence,setupSteps=componentRows.map(x=>x[0]+' '+x[1]+'/'+x[2]).join(' • ');
  const reasonUpper=reason.toUpperCase();let missingCondition='Confluence: BUY '+Math.round(Number(scores.BUY)||0)+'/100 • SELL '+Math.round(Number(scores.SELL)||0)+'/100 • lead '+lead;
  if(recovering)missingCondition='استعادة بيانات السعر الحي';else if(cooldown)missingCondition='فترة حماية بعد الصفقة السابقة';else if(/USD_NEWS_BLACKOUT|NEWS RISK/.test(reasonUpper))missingCondition='فلتر الأخبار يمنع الدخول مؤقتًا';else if(/TARGET_ROOM_TOO_SMALL|TP1_BELOW_0_5R/.test(reasonUpper))missingCondition='المسافة إلى الهدف أو العائد مقابل المخاطرة غير كافية';else if(/STOP_TOO_|INVALID_MOMENTUM_STOP|MIN_LOT_EXCEEDS/.test(reasonUpper))missingCondition='وقف الخسارة أو حجم المخاطرة غير مناسب';else if(confidence<65)missingCondition='مجموع الأدلة أقل من 65/100 — ننتظر توافقًا أقوى';else if(lead<8)missingCondition='BUY و SELL متقاربان — لا اتجاه واضح بعد';else if(status==='CANDIDATE')missingCondition=candidateLocked?'خطة Confluence مثبتة — Entry / SL / Targets ثابتة؛ ننتظر دخول السعر منطقة التنفيذ':'Confluence مكتمل — ننتظر دخول السعر منطقة التنفيذ';
  const developingSide=['BUY','SELL'].includes(raw?.candidateAction)?raw.candidateAction:null;
  const scenarioLabel=active?(side+' مؤكد — Confluence '+confidence+'/100'):candidate?((candidateLocked?'مرشح مثبت ':'مرشح ')+candidateSide+' — Confluence '+confidence+'/100'):recovering?'استعادة البيانات':cooldown?'انتظار بعد الصفقة':(developingSide?('Confluence building — '+developingSide):'انتظار');
  const waitReason=recovering?'جاري استعادة بيانات السعر الحي.':cooldown?'فترة حماية قصيرة بعد الصفقة السابقة.':candidate?(candidateLocked?'الخطة مثبتة؛ ننتظر دخول السعر منطقة التنفيذ بدون تغيير Entry/SL/Targets.':'Confluence مكتمل؛ ننتظر دخول السعر منطقة التنفيذ.'):missingCondition;
  const plan={serverOwned:true,signalId:active?raw.signalId:null,state:stale?'STALE':active?(side==='BUY'?'UP':'DOWN'):collecting?'COLLECTING':'WAIT',scenarioLabel,confidence,minConfidence,setupProgress,setupSteps,missingCondition,locked:active,candidate,candidateLocked,entry:(active||candidate)?entry:null,invalidation:(active||candidate)?stop:null,target1:(active||candidate)?targets[0]:null,target2:(active||candidate)?targets[1]:null,target3:(active||candidate)?targets[2]:null,target4:(active||candidate)?targets[3]:null,targetLabels:Array.isArray(raw?.targetLabels)?raw.targetLabels:[],lotSizing:raw?.lotSizing||null,priceAction:raw?.priceAction||null,technicalRead:raw?.technicalRead||null,scenarioPlan,momentum,ict,entryLow:(active||candidate)?positive(raw.entryLow):null,entryHigh:(active||candidate)?positive(raw.entryHigh):null,tp1Hit:active&&Boolean(raw?.targetHits?.[0]),tp2Hit:active&&Boolean(raw?.targetHits?.[1]),tp3Hit:active&&Boolean(raw?.targetHits?.[2]),tp4Hit:active&&Boolean(raw?.targetHits?.[3]),lockCreatedAt:active?raw.issuedAtMs:null,eta1:null,eta2:null,sampleCount:Number(raw?.sampleCount)||0,spanMinutes:Math.min(60,(Number(raw?.readingCompleteness)||0)*.6),channel:active?(side==='BUY'?'RISING':'FALLING'):'FLAT',note:active?('Confluence '+String(raw?.strategy||'SETUP')+' • BUY '+Math.round(Number(scores.BUY)||0)+'/100 • SELL '+Math.round(Number(scores.SELL)||0)+'/100'):candidate?(candidateLocked?('Confluence plan locked • '+candidateSide+' • Entry/SL/Targets ثابتة'):('Confluence ready • '+candidateSide+' • '+confidence+'/100')):waitReason};
  ...
  source = source.replace(
    ...
    'XAUUSD — MULTI-MODEL CONFLUENCE | Structure + Trend + Momentum + Price Action + Liquidity'
  );
  source = source.replace(
  ...
  source = source.replace(
    ...
    'المحرك يجمع Structure + Trend + Momentum + Price Action + Liquidity + Fibonacci؛ لا توجد مدرسة واحدة تتحكم بالقرار.'
  );
  ...
  source = source.replace(
    ...
    '<div class="goldPlanCard"><span>تقدم الإشارة</span><strong id="goldSetupProgress">0%</strong><small id="goldSetupSteps">Structure — • Trend — • Momentum — • Price Action — • Liquidity — • Fib —</small></div><div class="goldPlanCard"><span>الشرط الناقص الآن</span><strong id="goldMissingCondition">—</strong><small>يتحدث مع كل قراءة جديدة</small></div><div class="goldPlanCard"><span>اللوت المحسوب</span><strong id="goldLotSize">—</strong><small id="goldLotRisk">حسب مسافة SL</small></div><div class="goldPlanCard"><span>Liquidity / Structure Context</span><strong id="goldIctDraw">—</strong><small id="goldIctBias">Context only • not a hard gate</small></div><div class="goldPlanCard"><span>Price Action / Reversal</span><strong id="goldPatternRead">—</strong><small id="goldPatternState">Wedge • Triangle • H&S • HH/HL • LH/LL</small></div><div class="goldPlanCard"><span>Technical Confluence</span><strong id="goldTechnicalRead">—</strong><small id="goldTechnicalState">MA50/200 • RSI14 • MACD • Stoch 5,3,3 • Candle</small></div><div class="goldPlanCard"><span>إلغاء السيناريو</span><strong id="goldInvalidation">—</strong><small id="goldModelWindow">بيانات الرصد: —</small></div>'
  );
  source = source.replace(
