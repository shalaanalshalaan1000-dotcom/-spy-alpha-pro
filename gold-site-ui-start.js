...
function patchSiteSignalUi(source) {
  ...
  const checks=[['15m Trend',trendReady],['5m Momentum',momentumReady],['RSI',rsiReady],['Entry Zone',entryReady]],done=checks.filter(x=>x[1]).length,setupProgress=active?100:collecting?Math.min(15,Math.round((Number(raw?.sampleCount)||0)/120*15)):Math.round(done/checks.length*100),setupSteps=checks.map(x=>x[0]+' '+(x[1]?'✓':'—')).join(' • ');
  const reasonUpper=reason.toUpperCase();let missingCondition='بانتظار اتجاه وزخم واضحين';
  if(recovering)missingCondition='استعادة بيانات السعر الحي';else if(cooldown)missingCondition='فترة حماية بعد الصفقة السابقة';else if(confidence>0&&confidence<minConfidence)missingCondition='الثقة '+Math.round(confidence)+'% أقل من المطلوب '+Math.round(minConfidence)+'%';else if(/NO CHASE/.test(reasonUpper))missingCondition='السعر ابتعد عن منطقة الدخول؛ ننتظر فرصة جديدة';else if(/STOP_TOO_|INVALID_MOMENTUM_STOP|MIN_LOT_EXCEEDS/.test(reasonUpper))missingCondition='وقف الخسارة أو حجم المخاطرة غير مناسب';else if(/MAIN_TARGET_BELOW_MIN_R|TP1_TOO_CLOSE/.test(reasonUpper))missingCondition='المسافة إلى الهدف الأول غير كافية';else if(/USD_NEWS_BLACKOUT|NEWS RISK/.test(reasonUpper))missingCondition='فلتر الأخبار يمنع الدخول مؤقتًا';else if(status==='CANDIDATE')missingCondition='الصفقة مرشحة وتنتظر دخول السعر داخل منطقة التنفيذ';
  const scenarioLabel=active?(side==='BUY'?'BUY مؤكد':'SELL مؤكد'):candidate?('مرشح '+candidateSide+' — Trend/Momentum'):recovering?'استعادة البيانات':cooldown?'انتظار بعد الصفقة':candidateSide?('قيد البناء '+candidateSide):'انتظار';
  const waitReason=recovering?'جاري استعادة بيانات السعر الحي.':cooldown?'فترة حماية قصيرة بعد الصفقة السابقة.':candidate?'مرشح Trend/Momentum — ينتظر منطقة التنفيذ واعتماد الخادم.':missingCondition;
  const plan={serverOwned:true,signalId:active?raw.signalId:null,state:stale?'STALE':active?(side==='BUY'?'UP':'DOWN'):collecting?'COLLECTING':'WAIT',scenarioLabel,confidence,minConfidence,setupProgress,setupSteps,missingCondition,locked:active,candidate,entry:(active||candidate)?entry:null,invalidation:(active||candidate)?stop:null,target1:(active||candidate)?targets[0]:null,target2:(active||candidate)?targets[1]:null,target3:(active||candidate)?targets[2]:null,target4:(active||candidate)?targets[3]:null,targetLabels:Array.isArray(raw?.targetLabels)?raw.targetLabels:[],lotSizing:raw?.lotSizing||null,momentum,entryLow:(active||candidate)?positive(raw.entryLow):null,entryHigh:(active||candidate)?positive(raw.entryHigh):null,tp1Hit:active&&Boolean(raw?.targetHits?.[0]),tp2Hit:active&&Boolean(raw?.targetHits?.[1]),tp3Hit:active&&Boolean(raw?.targetHits?.[2]),tp4Hit:active&&Boolean(raw?.targetHits?.[3]),lockCreatedAt:active?raw.issuedAtMs:null,eta1:null,eta2:null,sampleCount:Number(raw?.sampleCount)||0,spanMinutes:Math.min(5,(Number(raw?.readingCompleteness)||0)/20),channel:active?(side==='BUY'?'RISING':'FALLING'):'FLAT',note:active?('Trend/Momentum '+String(raw?.strategy||'SETUP')+' • 15m '+String(raw?.contextBias||'—')+' • RSI '+(Number.isFinite(rsi5)?rsi5.toFixed(1):'—')):candidate?('مرشح '+candidateSide+' '+confidence+'% • Score gap '+(Number.isFinite(gap)?gap.toFixed(1):'—')+' • RSI '+(Number.isFinite(rsi5)?rsi5.toFixed(1):'—')):waitReason};
  ...
  source = source.replace(
    ...
    'Trend Momentum XAUUSD — 15m + 5m + 1m'
  );
  source = source.replace(
...
(function(){
 ...
 function missing(raw,conf,min){
  ...
  if(st==='CANDIDATE')return 'مرشح Trend/Momentum وينتظر دخول منطقة التنفيذ';
  return 'بانتظار اتجاه وزخم واضحين';
 }
 async function refresh(){
  try{
   ...
   const mom=raw?.momentum||raw?.prediction||{},gap=Number(mom?.scoreGap??raw?.prediction?.scoreGap??0),rsi=Number(mom?.rsi5),structure5=Number(mom?.structure5m??raw?.prediction?.structure5m??0),pressure5=Number(mom?.pressure5m??raw?.prediction?.pressure5m??0);
   ...
   set('goldIctDraw','Score gap '+(Number.isFinite(gap)?gap.toFixed(1):'—')+' • RSI '+(Number.isFinite(rsi)?rsi.toFixed(1):'—'));set('goldIctBias','15m: '+String(raw?.contextBias||'NEUTRAL')+' • 5m structure '+(Number.isFinite(structure5)?structure5:'—'));
