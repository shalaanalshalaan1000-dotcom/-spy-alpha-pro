...
function patchSiteSignalUi(source) {
  ...
  const contextReady=Number(ict?.dir15)!==0||Number(ict?.dir1)!==0,sweepReady=Boolean(ict?.legSweep||ict?.sweep),shiftReady=Boolean(ict?.mss||ict?.displacement||ict?.shift1||ict?.shift5||ict?.shift15),fvgReady=Boolean(ict?.originFvg||ict?.fvg1||ict?.fvg5||ict?.fvg),targetReady=Boolean(ict?.mainLiquidity||raw?.target1),triggerReady=active||candidate||Boolean(ict?.executionReady);
...
(function(){
 ...
 async function refresh(){
  try{
   ...
   const ict=raw?.ict||{},contextReady=Number(ict?.dir15)!==0||Number(ict?.dir1)!==0,sweepReady=Boolean(ict?.legSweep||ict?.sweep),shiftReady=Boolean(ict?.mss||ict?.displacement||ict?.shift1||ict?.shift5||ict?.shift15),fvgReady=Boolean(ict?.originFvg||ict?.fvg1||ict?.fvg5||ict?.fvg),targetReady=Boolean(ict?.mainLiquidity||raw?.target1),triggerReady=active||status==='CANDIDATE'||Boolean(ict?.executionReady);
