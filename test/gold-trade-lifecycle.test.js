...
test('a genuinely new structure can create a new entry after cooldown', () => {
  ...
  processSignalLifecycle(s,{model,quote:q,now:start+61_000,execute:true});
...
test('SELL entry uses bid and target order is retained', () => {
  ...
  const s=state(createSignal(model,start)),q=quote(100.2,{bid:100.0,ask:100.7,t:start+1_000});
