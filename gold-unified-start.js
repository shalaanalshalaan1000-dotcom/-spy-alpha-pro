...
#alphaAutoDock h3{margin:0 0 10px;color:#67e2b2}.alphaAutoGrid{display:grid;grid-template-columns:repeat(4,1fr);gap:8px}.alphaAutoBox{padding:10px;border:1px solid #30445d;border-radius:11px;background:#0d1420}.alphaAutoBox span{display:block;color:#91a0b8;font-size:11px}.alphaAutoBox strong{display:block;margin-top:5px}.alphaAutoOk{color:#54e0a4}.alphaAutoBad{color:#ff718c}.alphaAutoWarn{color:#ffd166}@media(max-width:760px){.alphaAutoGrid{grid-template-columns:repeat(2,1fr)}}
  ...
  const panel=`<section style="margin:14px 0;text-align:left"><a href="/spx" style="display:inline-block;padding:10px 14px;border:1px solid #5275ad;border-radius:12px;color:#b9d1ff;text-decoration:none">فتح جدول SPX اللحظي</a></section><section id="alphaAutoDock"><h3>الرصد الآلي — Gold Alpha</h3><div class="alphaAutoGrid"><div class="alphaAutoBox"><span>حالة الروبوت</span><strong id="aaConn" class="alphaAutoWarn">جاري الفحص</strong></div><div class="alphaAutoBox"><span>الإشارة التنفيذية</span><strong id="aaAction" class="alphaAutoWarn">WAIT</strong></div><div class="alphaAutoBox"><span>نطاق الدخول</span><strong id="aaRange">—</strong></div><div class="alphaAutoBox"><span>دورة الصفقة</span><strong id="aaLife">—</strong></div><div class="alphaAutoBox"><span>الأهداف المحققة</span><strong id="aaTargets">—</strong></div><div class="alphaAutoBox"><span>آخر نتيجة</span><strong id="aaTerminal">—</strong></div><div class="alphaAutoBox"><span>مصدر السعر</span><strong id="aaProvider">—</strong></div><div class="alphaAutoBox"><span>سبب القرار</span><strong id="aaReason">—</strong></div></div></section>`;
  const js=`<script>
(function(){
 const m=v=>Number.isFinite(Number(v))?String.fromCharCode(36)+Number(v).toFixed(2):'—';
 async function aa(){try{const [sr,st]=await Promise.all([fetch('/api/auto-trade/signal?observe=1',{cache:'no-store'}),fetch('/api/auto-trade/status',{cache:'no-store'})]);if(!sr.ok||!st.ok)throw new Error('engine unavailable');const s=await sr.json(),x=await st.json();const c=document.getElementById('aaConn'),a=document.getElementById('aaAction'),r=document.getElementById('aaRange'),n=document.getElementById('aaReason');if(!c)return;const connected=!!x.mt5?.connected;c.textContent=connected?'MT5 متصل':'MT5 غير متصل';c.className=connected?'alphaAutoOk':'alphaAutoBad';const act=['BUY','SELL'].includes(s.action)?s.action:'WAIT';a.textContent=act;a.className=act==='BUY'?'alphaAutoOk':act==='SELL'?'alphaAutoBad':'alphaAutoWarn';r.textContent=s.entryLow?m(s.entryLow)+' — '+m(s.entryHigh):'—';document.getElementById('aaLife').textContent=(s.status||'WAIT')+' • '+(s.entryConfirmation||'—');document.getElementById('aaTargets').textContent=(s.targetHits||[]).map((v,i)=>v?'TP'+(i+1):'—').filter(v=>v!=='—').join(' • ')||'لا شيء';document.getElementById('aaTerminal').textContent=x.lastTerminal?.outcome||'—';document.getElementById('aaProvider').textContent=(s.provider||'—')+(s.degraded?' • متأخر':'');n.textContent=s.reason||'—';}catch(e){const c=document.getElementById('aaConn');if(c){c.textContent='تعذر تحديث المحرك';c.className='alphaAutoBad';}}}
