...
async function getQuote() {
  ...
  try {
    const r = await fetch('https://api.gold-api.com/price/XAU', {
      cache:'no-store',
      headers:{accept:'application/json','user-agent':'GoldAlphaPro/6.0'},
...
async function signal(execute) {
  ...
  if (s) {
    ...
    if (stopped || done || expired) {
      ...
      state.signal = null;
      state.cooldownUntil = now + 90_000;
      s = null;
    }
  }

  if (!s && execute && now>=state.cooldownUntil &&
      ['BUY','SELL'].includes(model.candidateAction) && model.confidence>=MIN_CONFIDENCE &&
      model.entryLow && model.entryHigh) {
    s = {
  ...
  if (s) {
    ...
    return {
      ...
      readingCompleteness:model.readingCompleteness,
      signalConfidence:s.confidence,
      confidence:s.confidence,
      provider:q.provider,degraded:q.degraded,
      updatedAt:new Date(now).toISOString(),
      reason:active ? (inRange?'جاهز للتنفيذ على MT5':'الإشارة فعالة لكن السعر خارج نطاق الدخول') : 'إدارة الإشارة القائمة'
    };
  }

  return {
    ...model,
    signalConfidence:model.confidence,
    provider:q.provider,degraded:q.degraded,
    executionMode:'XAUUSD_ONLY',
    reason: now<state.cooldownUntil ? 'مهلة 90 ثانية بعد اكتمال السيناريو السابق' : model.reason
  };
}
...
<title>Gold Alpha Pro</title>
<style>
:root{font-family:Inter,system-ui,-apple-system,"Segoe UI",sans-serif;background:#080b12;color:#eef2ff}*{box-sizing:border-box}body{margin:0;background:radial-gradient(circle at top,#111a2d,#080b12 42%);min-height:100vh}header{padding:18px max(14px,4vw);border-bottom:1px solid #26324a;background:#0c111ddd;position:sticky;top:0;z-index:2}h1{margin:0;font-size:25px;color:#f4dfa0}header p{margin:5px 0 0;color:#9da9c3;font-size:12px}main{max-width:1200px;margin:auto;padding:16px}.tag{display:inline-block;padding:5px 9px;border:1px solid #8f7334;border-radius:999px;color:#f4d36d;font-size:11px;font-weight:900}.grid{display:grid;grid-template-columns:repeat(4,1fr);gap:9px;margin-bottom:12px}.card,.panel{background:#0e1524;border:1px solid #273550;border-radius:15px;padding:13px}.card span,.muted{color:#8ea0c0;font-size:11px}.card strong{display:block;margin-top:6px;font-size:20px;direction:ltr}.gold{color:#f5d773}.green{color:#52e5a5}.red{color:#ff718c}.warn{color:#ffd166}.panel{margin-bottom:12px}.panel h2{margin:0 0 12px;font-size:16px}.auto{border-color:#2e6654;background:linear-gradient(145deg,#0f1d19,#0c121c)}.auto h2{color:#72dfb5}.reading{display:grid;grid-template-columns:1fr 1fr;gap:9px}.meter{height:9px;background:#1b263a;border-radius:999px;overflow:hidden;margin-top:8px}.meter>i{display:block;height:100%;background:#52e5a5;width:0}.meter.conf>i{background:#ffd166}.targets{display:grid;grid-template-columns:repeat(4,1fr);gap:8px}.target{padding:10px;border:1px solid #334155;border-radius:10px;background:#0b1220}.target span{font-size:10px;color:#8ea0c0}.target strong{display:block;margin-top:5px;direction:ltr}.chart{height:420px;border-radius:14px;overflow:hidden;border:1px solid #273550;margin-bottom:12px}.chart iframe{width:100%;height:100%;border:0}.tableWrap{overflow:auto}.trades{width:100%;border-collapse:collapse;min-width:850px}.trades th,.trades td{padding:9px;border-bottom:1px solid #273550;text-align:right;font-size:12px;white-space:nowrap}.trades th{color:#8ea0c0}.empty{text-align:center!important;color:#8ea0c0}.status{display:inline-block;padding:4px 8px;border-radius:999px;border:1px solid #334155;font-size:10px}@media(max-width:800px){.grid{grid-template-columns:repeat(2,1fr)}.reading{grid-template-columns:1fr}.targets{grid-template-columns:repeat(2,1fr)}.chart{height:360px}}
</style></head><body>
<header><h1>Gold Alpha Pro</h1><p>XAUUSD ONLY • AUTO TRADING • BTC OFF</p></header>
<main>
<section class="grid">
<div class="card"><span>سعر الذهب</span><strong id="price" class="gold">—</strong></div>
<div class="card"><span>الاتجاه القصير</span><strong id="direction">—</strong></div>
<div class="card"><span>اكتمال القراءة</span><strong id="completion" class="green">0%</strong></div>
<div class="card"><span>ثقة الإشارة الحالية</span><strong id="confidence" class="warn">0%</strong></div>
</section>

<section class="panel reading">
<div><div class="muted">اكتمال القراءة — لا ينخفض بعد اكتمال 5 شموع M1</div><div class="meter"><i id="completionBar"></i></div><p id="readingNote" class="muted">جاري جمع البيانات…</p></div>
<div><div class="muted">ثقة الإشارة — تتغير مع السوق</div><div class="meter conf"><i id="confidenceBar"></i></div><p id="signalNote" class="muted">لا توجد إشارة حاليًا</p></div>
</section>

<section class="panel auto">
<h2>XAUUSD — التداول الآلي</h2>
<section class="grid">
<div class="card"><span>MT5</span><strong id="mt5">غير متصل</strong></div>
<div class="card"><span>الحالة</span><strong id="action">WAIT</strong></div>
<div class="card"><span>نطاق الدخول</span><strong id="range">—</strong></div>
<div class="card"><span>وقف الخسارة</span><strong id="sl">—</strong></div>
</section>
<div class="targets">
<div class="target"><span>TP1</span><strong id="tp1">—</strong></div>
<div class="target"><span>TP2</span><strong id="tp2">—</strong></div>
<div class="target"><span>TP3</span><strong id="tp3">—</strong></div>
<div class="target"><span>TP4</span><strong id="tp4">—</strong></div>
</div>
<p id="autoNote" class="muted">يتم تحديث حالة المحرك كل 5 ثوانٍ.</p>
</section>

<div class="chart"><iframe src="https://s.tradingview.com/widgetembed/?symbol=OANDA%3AXAUUSD&interval=5&theme=dark&style=1&timezone=Asia%2FRiyadh&hideideas=1"></iframe></div>

<section class="panel">
<h2>سجل صفقات MT5 الفعلية</h2>
<div class="tableWrap"><table class="trades"><thead><tr><th>الحالة</th><th>النوع</th><th>الدخول</th><th>الحجم</th><th>SL</th><th>TP1</th><th>TP2</th><th>TP3</th><th>TP4</th><th>النتيجة</th><th>الربح/الخسارة</th><th>الوقت</th></tr></thead><tbody id="tradeRows"><tr><td colspan="12" class="empty">لا توجد صفقات منفذة مسجلة بعد</td></tr></tbody></table></div>
<p class="muted">هذا السجل يعرض فقط الأحداث التي يرجعها مستشار MT5 إلى الموقع؛ الإشارة وحدها لا تُحسب صفقة منفذة.</p>
</section>
</main>
<script>
const $=s=>document.querySelector(s), money=v=>Number.isFinite(Number(v))?'$'+Number(v).toFixed(2):'—';
let completionHigh=0;
async function refresh(){
  try{
    const [sr,tr]=await Promise.all([fetch('/api/auto-trade/signal?observe=1',{cache:'no-store'}),fetch('/api/auto-trade/status',{cache:'no-store'})]);
    const s=await sr.json(), st=await tr.json();
    completionHigh=Math.max(completionHigh,Number(s.readingCompleteness||0));
    const complete=Math.min(100,completionHigh), conf=Math.max(0,Math.min(100,Number(s.signalConfidence??s.confidence??0)));
    $('#price').textContent=money(s.price);
    $('#completion').textContent=complete+'%'; $('#completionBar').style.width=complete+'%';
    $('#confidence').textContent=conf+'%'; $('#confidenceBar').style.width=conf+'%';
    $('#direction').textContent=s.candidateAction==='BUY'?'صاعد':s.candidateAction==='SELL'?'هابط':'متوازن';
    $('#readingNote').textContent=complete>=100?'القراءة مكتملة 100% — تستمر مراقبة السوق':'جمع بيانات M1: '+Number(s.barCount||0)+'/5';
    $('#signalNote').textContent=s.reason||'—';
    const act=s.candidateAction||'WAIT'; $('#action').textContent=act;
    $('#action').className=act==='BUY'?'green':act==='SELL'?'red':'warn';
    $('#range').textContent=s.entryLow?money(s.entryLow)+' — '+money(s.entryHigh):'—';
    $('#sl').textContent=money(s.stopLoss); $('#tp1').textContent=money(s.target1); $('#tp2').textContent=money(s.target2); $('#tp3').textContent=money(s.target3); $('#tp4').textContent=money(s.target4);
    const connected=!!st.mt5?.connected; $('#mt5').textContent=connected?(st.mt5.tradingEnabled?'متصل • التداول مفعل':'متصل • مراقبة'):'غير متصل';
    $('#mt5').className=connected?'green':'red';
    $('#autoNote').textContent=(s.status||'WAIT')+' • '+(s.strategy||'READING')+' • '+(s.reason||'');
    renderTrades(st.trades||[]);
  }catch(e){ $('#autoNote').textContent='تعذر تحديث المحرك: '+e.message; }
}
function renderTrades(list){
 const body=$('#tradeRows'); if(!list.length){body.innerHTML='<tr><td colspan="12" class="empty">لا توجد صفقات منفذة مسجلة بعد</td></tr>';return}
 body.innerHTML=list.slice().reverse().map(t=>'<tr>'+ 
 '<td><span class="status">'+(t.status||t.type||'—')+'</span></td>'+ 
 '<td>'+(t.side||'—')+'</td><td>'+money(t.entry)+'</td><td>'+(t.volume??'—')+'</td><td>'+money(t.stopLoss||t.sl)+'</td>'+ 
 '<td>'+(t.tp1Hit?'✓ ':'')+money(t.target1||t.tp1)+'</td><td>'+(t.tp2Hit?'✓ ':'')+money(t.target2||t.tp2)+'</td>'+ 
 '<td>'+(t.tp3Hit?'✓ ':'')+money(t.target3||t.tp3)+'</td><td>'+(t.tp4Hit?'✓ ':'')+money(t.target4||t.tp4)+'</td>'+ 
 '<td>'+(t.closeReason||t.result||'—')+'</td><td>'+(Number.isFinite(Number(t.profit))?Number(t.profit).toFixed(2):'—')+'</td>'+ 
 '<td>'+(t.closedAt||t.openedAt||t.updatedAt||'—')+'</td></tr>').join('');
}
...
const server=http.createServer(async(req,res)=>{
  ...
  try{
    ...
    if(req.method==='GET'&&url.pathname==='/api/health') return json(res,200,{ok:true,service:'gold-alpha-pro',btc:false});
    ...
    if(req.method==='GET'&&url.pathname==='/api/auto-trade/status'){
      ...
      return json(res,200,{mt5:{...state.mt5,connected},trades:state.trades});
    }
...
server.listen(PORT,'0.0.0.0',()=>console.log(`Gold Alpha Pro listening on ${PORT}`));
