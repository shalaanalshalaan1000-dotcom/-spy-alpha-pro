...
input double RiskPercent          = 0.75;      // Conservative maximum equity risk per trade.
input double MaxDailyLossPercent  = 3.00;      // Shared account stop for this EA's instances.
input double MaxSpreadPrice       = 0.00;      // Optional manual override in symbol price units. 0 = use asset-specific limits below.
input double MaxGoldSpreadPrice   = 0.80;      // Automatic XAUUSD spread limit in USD.
input double MaxCryptoSpreadPct   = 0.10;      // Automatic BTCUSD spread limit as % of price (0.10 = 0.10%).
input int    RetryExpiredSignalSeconds = 0;    // Never open a trade from an expired signal.
input int    PollSeconds          = 20;        // Normal polling interval.
input int    RequestTimeoutMs     = 15000;
input int    MaxConsecutiveSignalFailures = 3; // Suspend new entries after this many failures until recovery.
input int    BackoffBaseSeconds   = 15;
input int    BackoffMaxSeconds    = 120;
...
int g_signal_failures = 0;
datetime g_next_signal_attempt = 0;
bool g_feed_healthy = true;
...
int BackoffSeconds(){int power=MathMax(g_signal_failures-1,0);double wait=(double)MathMax(BackoffBaseSeconds,1)*MathPow(2.0,MathMin(power,6));return (int)MathMin(wait,(double)MathMax(BackoffMaxSeconds,BackoffBaseSeconds));}
void RegisterSignalFailure(const string reason){g_signal_failures++;g_feed_healthy=false;int wait=BackoffSeconds();g_next_signal_attempt=TimeLocal()+wait;Print("Signal feed failure #",g_signal_failures,": ",reason,". Retry in ",wait,"s.");if(g_signal_failures==MaxConsecutiveSignalFailures)Print("AUTO ENTRY SUSPENDED: signal feed is unhealthy. New trades remain blocked until a successful HTTP 200 response.");}
void RegisterSignalSuccess(){if(g_signal_failures>0)Print("Signal feed recovered after ",g_signal_failures," failure(s). New entries may resume if trading settings and signal validation allow them.");g_signal_failures=0;g_next_signal_attempt=0;g_feed_healthy=true;}

bool FetchSignal(string &json){if(TimeLocal()<g_next_signal_attempt)return false;char payload[],response[];string headers;ResetLastError();int status=WebRequest("GET",g_signal_url,"Accept: application/json\r\n",RequestTimeoutMs,payload,response,headers);int last_error=GetLastError();string body=CharArrayToString(response,0,-1,CP_UTF8);if(status==-1){RegisterSignalFailure("WebRequest failed; MT5 error "+IntegerToString(last_error)+". Check allowed URL and network");return false;}if(status!=200){string preview=StringSubstr(body,0,180);RegisterSignalFailure("unexpected response code "+IntegerToString(status)+(preview!=""?"; body: "+preview:""));return false;}if(body==""){RegisterSignalFailure("HTTP 200 with empty response body");return false;}json=body;RegisterSignalSuccess();return true;}
...
double AllowedSpread(const double price){if(MaxSpreadPrice>0.0)return MaxSpreadPrice;string upper=g_symbol;StringToUpper(upper);if(StringFind(upper,"BTC")>=0)return price*(MathMax(MaxCryptoSpreadPct,0.0)/100.0);return MathMax(MaxGoldSpreadPrice,0.0);}
