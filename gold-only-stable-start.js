...
// Persistent target-hit tracking. A completed setup is now CONSUMED and cannot
// be re-entered from the same levels. A new same-direction setup is allowed only
// after a 10-minute cooldown AND a material structural change in its levels.
const renderAnchor='function renderGoldPlan(plan){';
if(source.includes(renderAnchor)){
  const lockEngine=`const GOLD_TRADE_LOCK_KEY='gold_alpha_trade_lock_v3',GOLD_TRADE_LOCK_LEGACY_KEY='gold_alpha_trade_lock_v2',GOLD_TRADE_COOLDOWN_KEY='gold_alpha_trade_cooldown_v3',GOLD_TRADE_COMPLETED_KEY='gold_alpha_trade_completed_v3';
const GOLD_REENTRY_COOLDOWN_MS=10*60_000,GOLD_COMPLETED_RETENTION_MS=4*60*60_000,GOLD_FRESH_LEVEL_SHIFT=3.0;
...
function lockGoldPlan(plan,price){const p=Number(price),active=['UP','DOWN'].includes(plan?.state);let lock=readGoldTradeLock();if(lock){const invalidation=Number(lock.invalidation),invalidated=Number.isFinite(invalidation)&&(lock.state==='UP'?p<=invalidation:p>=invalidation),opposite=active&&plan.state!==lock.state,expired=Date.now()-Number(lock.createdAt||0)>4*60*60_000;lock.tp1Hit=Boolean(lock.tp1Hit)||reachedGoldLevel(lock.state,p,lock.target1);lock.tp2Hit=Boolean(lock.tp2Hit)||reachedGoldLevel(lock.state,p,lock.target2);if(lock.tp2Hit){setGoldCompleted(lock);clearGoldTradeLock();setGoldCooldown();return{...plan,entry:null,locked:false,consumed:true,cooling:true,tradeCompleted:true,tp1Hit:true,tp2Hit:true,completedTrade:lock,note:'✓ اكتملت الصفقة — السيناريو مستهلك. ممنوع إعادة الدخول من نفس النقطة حتى تتكوّن إشارة جديدة.'}}if(invalidated||opposite||expired){clearGoldTradeLock();lock=null}else writeGoldTradeLock(lock)}
const completed=readGoldCompleted(),cooling=Date.now()<readGoldCooldown(),fresh=isFreshAfterCompleted(plan,completed);if(!lock&&!cooling&&fresh&&active&&Number.isFinite(p)&&Number.isFinite(Number(plan.target1))&&Number.isFinite(Number(plan.target2))&&Number.isFinite(Number(plan.invalidation))){lock={state:plan.state,entry:p,target1:Number(plan.target1),target2:Number(plan.target2),invalidation:Number(plan.invalidation),confidence:Number(plan.confidence||0),createdAt:Date.now(),tp1Hit:false,tp2Hit:false};writeGoldTradeLock(lock)}
if(!lock){const consumed=Boolean(completed&&!fresh);return{...plan,entry:null,locked:false,cooling,consumed,tradeCompleted:Boolean(completed),tp1Hit:Boolean(completed?.tp1Hit),tp2Hit:Boolean(completed?.tp2Hit),completedTrade:completed,note:consumed?'✓ السيناريو السابق مستهلك — لا إعادة دخول من نفس المستويات. بانتظار بنية جديدة.':cooling?'✓ الصفقة السابقة انتهت — إعادة تقييم السوق خلال فترة الحماية.':completed?'✓ الصفقة السابقة مكتملة — ظهرت بنية جديدة ويجري التحقق منها.':plan.note}}
...
function renderGoldHitState(plan){const t1=$('#goldTarget1'),t2=$('#goldTarget2'),note=$('#goldPlanNote');if(t1){t1.classList.toggle('goldHit',!!plan.tp1Hit);if(plan.tp1Hit&&!t1.textContent.includes('✓'))t1.textContent='✓ '+t1.textContent}if(t2){t2.classList.toggle('goldHit',!!plan.tp2Hit);if(plan.tp2Hit&&!t2.textContent.includes('✓'))t2.textContent='✓ '+t2.textContent}if(note&&(plan.tradeCompleted||plan.consumed))note.textContent=plan.note}
`;
...
source=source.replace(
  ...
  "function renderGoldPlan(plan){\n  const entryNode=$('#goldEntryLocked'),entryStatus=$('#goldEntryLockStatus');if(entryNode){entryNode.textContent=plan.locked?money(plan.entry):'—';entryNode.className=plan.locked?'locked':''}if(entryStatus)entryStatus.textContent=plan.consumed?'CONSUMED • ممنوع إعادة الدخول':plan.tradeCompleted?'✓ اكتملت الصفقة — بانتظار سيناريو جديد':plan.cooling?'حماية بعد الإغلاق • إعادة تقييم…':plan.locked?'LOCKED • صفقة واحدة فقط':'بانتظار إشارة مكتملة';\n  const labels="
);
