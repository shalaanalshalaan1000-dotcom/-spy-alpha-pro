...
function renderGoldHitState(plan){const t1=$('#goldTarget1'),t2=$('#goldTarget2'),note=$('#goldPlanNote'),status=$('#goldPlanStatus'),scenario=$('#goldScenario'),invalid=$('#goldInvalidation');if(plan.stopped){if(status){status.textContent='STOP LOSS HIT';status.className='goldPlanStatus goldStop'}if(scenario){scenario.textContent='انتهى';scenario.className='goldStop'}if(t1){t1.textContent='—';t1.classList.remove('goldHit')}if(t2){t2.textContent='—';t2.classList.remove('goldHit')}if(invalid){invalid.textContent='—';invalid.className=''}if(note)note.textContent=plan.note;return}if(t1){t1.classList.toggle('goldHit',!!plan.tp1Hit);if(plan.tp1Hit&&!t1.textContent.includes('✓'))t1.textContent='✓ '+t1.textContent}if(t2){t2.classList.toggle('goldHit',!!plan.tp2Hit);if(plan.tp2Hit&&!t2.textContent.includes('✓'))t2.textContent='✓ '+t2.textContent}if(note&&(plan.tradeCompleted||plan.consumed))note.textContent=plan.note}
`;
...
source=source.replace(
  ...
  "function renderGoldPlan(plan){\n  const entryNode=$('#goldEntryLocked'),entryStatus=$('#goldEntryLockStatus');if(entryNode){entryNode.textContent=plan.locked?money(plan.entry):'—';entryNode.className=plan.locked?'locked':''}if(entryStatus)entryStatus.textContent=plan.stopped?'⛔ STOP LOSS HIT • بانتظار سيناريو جديد':plan.consumed?'CONSUMED • ممنوع إعادة الدخول':plan.tradeCompleted?'✓ اكتملت الصفقة — بانتظار سيناريو جديد':plan.cooling?'حماية بعد الإغلاق • إعادة تقييم…':plan.locked?'LOCKED • صفقة واحدة فقط':'بانتظار إشارة مكتملة';\n  const labels="
);
