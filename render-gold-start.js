...
function patchRenderGoldLayout(source){
  const css=`
.goldPlanGrid.renderFinalGoldGrid{grid-template-columns:repeat(4,1fr)}
...
function syncRenderFinalGoldLayout(){
  ...
  const hits=Array.isArray(lock.tpHits)?lock.tpHits:[Boolean(lock.tp1Hit),Boolean(lock.tp2Hit),false,false],open=Boolean(hits[0]&&hits[1]);
...
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>{setTimeout(syncRenderFinalGoldLayout,700);setInterval(syncRenderFinalGoldLayout,1000)});else{setTimeout(syncRenderFinalGoldLayout,700);setInterval(syncRenderFinalGoldLayout,1000)}
`;
  if(!source.includes('function syncRenderFinalGoldLayout')&&source.includes('(async()=>{'))source=source.replace('(async()=>{',client+'\n(async()=>{');
  return source;
}
...
fs.writeFileSync=function(path,data,...args){
  ...
  const isBuffer=Buffer.isBuffer(data),patched=patchRenderGoldLayout(isBuffer?data.toString('utf8'):String(data));
