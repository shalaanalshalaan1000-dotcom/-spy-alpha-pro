...
if(source.includes(renderAnchor)){
  const phase1=`const GOLD_TRADE_LOCK_KEY='gold_alpha_trade_lock_v1',GOLD_TRADE_COOLDOWN_KEY='gold_alpha_trade_cooldown_v1';
...
function readGoldCooldown(){try{return JSON.parse(localStorage.getItem(GOLD_TRADE_COOLDOWN_KEY)||'null')}catch{return null}}
function writeGoldCooldown(x){try{localStorage.setItem(GOLD_TRADE_COOLDOWN_KEY,JSON.stringify(x))}catch{}}
function clearGoldCooldown(){try{localStorage.removeItem(GOLD_TRADE_COOLDOWN_KEY)}catch{}}
function goldStopTouched(lock,price,samples){const stop=Number(lock?.invalidation),p=Number(price);if(!Number.isFinite(stop))return false;const since=Number(lock?.createdAt||0),rows=(samples||[]).filter(x=>Number(x.t)>=since).map(x=>Number(x.price)).filter(Number.isFinite);rows.push(p);return lock.state==='UP'?rows.some(v=>v<=stop):rows.some(v=>v>=stop)}
...
function lockGoldPlan(plan,price,samples=[],reading=null){const p=Number(price),active=['UP','DOWN'].includes(plan?.state);let lock=readGoldTradeLock(),closedReason=null;if(lock){if(lock.target3==null||lock.target4==null||!Number.isFinite(Number(lock.target3))||!Number.isFinite(Number(lock.target4))){lock={...lock,...extendGoldTargets(lock.state,lock.entry,lock.target1,lock.target2)};writeGoldTradeLock(lock)}const renewed=renewGoldPlanAfterAllTargets(lock,plan,p);if(renewed!==lock){lock=renewed;writeGoldTradeLock(lock)}const observedFrom=Number(reading?.observedFromMs),historyMatches=Number.isFinite(observedFrom)&&observedFrom===Number(lock.createdAt),sideExtreme=historyMatches?(lock.state==='UP'?Number(reading?.observedLow):Number(reading?.observedHigh)):NaN,sideQuote=lock.state==='UP'?Number(reading?.bid):Number(reading?.ask),stopPrice=Number.isFinite(sideExtreme)&&sideExtreme>0?sideExtreme:Number.isFinite(sideQuote)&&sideQuote>0?sideQuote:p,invalidated=goldStopTouched(lock,stopPrice,samples),opposite=active&&plan.state!==lock.state,expired=Date.now()-Number(lock.createdAt||0)>4*60*60_000;if(invalidated||opposite||expired){closedReason=invalidated?'STOP_HIT':opposite?'DIRECTION_CHANGED':'EXPIRED';if(invalidated)writeGoldCooldown({state:lock.state,until:Date.now()+5*60_000,reason:closedReason});clearGoldTradeLock();lock=null}}let cooldown=readGoldCooldown();if(cooldown&&Date.now()>=Number(cooldown.until||0)){clearGoldCooldown();cooldown=null}const blocked=cooldown&&active&&plan.state===cooldown.state;if(!lock&&!closedReason&&!blocked&&active&&Number.isFinite(p)&&plan.target1!=null&&plan.target2!=null&&plan.invalidation!=null&&Number.isFinite(Number(plan.target1))&&Number.isFinite(Number(plan.target2))&&Number.isFinite(Number(plan.invalidation))){const extension=extendGoldTargets(plan.state,p,plan.target1,plan.target2);lock={state:plan.state,entry:p,target1:Number(plan.target1),target2:Number(plan.target2),target3:extension.target3,target4:extension.target4,invalidation:Number(plan.invalidation),confidence:Number(plan.confidence||0),cycle:1,createdAt:Date.now()};writeGoldTradeLock(lock)}if(!lock)return{...plan,state:'WAIT',currentPrice:p,entry:null,target1:null,target2:null,target3:null,target4:null,invalidation:null,locked:false,closedReason:closedReason||(blocked?'STOP_HIT':null),note:closedReason==='STOP_HIT'||blocked?'تم ضرب الوقف — أغلقت الإشارة، ويُمنع تكرار الاتجاه نفسه لمدة 5 دقائق.':plan.note};return{...plan,state:lock.state,confidence:Math.max(Number(plan.confidence||0),Number(lock.confidence||0)),currentPrice:p,entry:lock.entry,target1:lock.target1,target2:lock.target2,target3:lock.target3,target4:lock.target4,invalidation:lock.invalidation,locked:true,lockCreatedAt:lock.createdAt,note:Number(lock.cycle||1)>1?'تم تحقق الأهداف السابقة وإصدار أهداف جديدة بعد إعادة تأكيد الاتجاه • الجولة '+lock.cycle:'ENTRY LOCKED — بعد تحقق الأهداف الأربعة يعاد التأكيد وتصدر مجموعة أهداف جديدة تلقائيًا.'}}
...
source=source.replace('renderGoldPlan(d.plan);','renderGoldPlan(lockGoldPlan(d.plan,d.price,goldSamples,d));drawIctLiquidityLevels(goldSamples,d.price);');
