...
function renewGoldPlanAfterAllTargets(lock,plan,price){const p=Number(price),sameDirection=plan?.state===lock?.state,confirmed=sameDirection&&Number(plan?.confidence)>=75,allHit=[lock?.target1,lock?.target2,lock?.target3,lock?.target4].every(target=>goldTargetReached(lock?.state,p,target));if(!allHit||!confirmed)return lock;const sign=lock.state==='UP'?1:-1,fresh1=Number(plan.target1),fresh2=Number(plan.target2),freshAhead=[fresh1,fresh2].every(Number.isFinite)&&(lock.state==='UP'?fresh1>p&&fresh2>fresh1:fresh1<p&&fresh2<fresh1),oldStep=Math.max(.05,Math.abs(Number(lock.target2)-Number(lock.target1))),target1=freshAhead?fresh1:p+sign*oldStep,target2=freshAhead?fresh2:p+sign*oldStep*1.6,extension=extendGoldTargets(lock.state,p,target1,target2),freshStop=Number(plan.invalidation),stopValid=Number.isFinite(freshStop)&&(lock.state==='UP'?freshStop<p:freshStop>p);return{...lock,entry:p,target1:Number(target1.toFixed(2)),target2:Number(target2.toFixed(2)),target3:extension.target3,target4:extension.target4,invalidation:stopValid?freshStop:lock.invalidation,confidence:Number(plan.confidence),cycle:Number(lock.cycle||1)+1,createdAt:Date.now(),renewedAt:Date.now()}}
