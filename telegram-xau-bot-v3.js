...

{
  ...
  const newTarget="const labels=Array.isArray(s?.targetLabels)?s.targetLabels:[],targetLines=t.map((v,i)=>valid(v)?\`🎯 TP\${i+1}: \${n(v)}\${labels[i]?' • '+labels[i]:''}\`:null).filter(Boolean);const cf=s?.confluence||{},scores=cf.scores||{},lot=s?.lotSizing||null,cfLines=[\`🧠 MULTI-MODEL CONFLUENCE\`,\`🧭 \${String(s?.strategy||'MULTI_MODEL_CONFLUENCE')}\`,\`📊 BUY \${Math.round(Number(scores.BUY)||0)}/100 • SELL \${Math.round(Number(scores.SELL)||0)}/100\`,\`🔎 Structure + Trend + Momentum + Price Action + Liquidity + Fib\`,lot&&Number(lot.recommendedLot)>0?\`📐 اللوت المحسوب: \${Number(lot.recommendedLot).toFixed(2)} lot • Risk ≈ $\${Number(lot.actualRiskUsd||0).toFixed(2)}\`:null].filter(Boolean);return \`\${icon} XAUUSD — \${side}\\n✅ CONFLUENCE CONFIRMED\\n📊 الثقة: \${confidence}%\\n💵 الدخول: \${money(entry)}\\n🛑 SL: \${n(sl)}\\n\${targetLines.join('\\n')}\\n\\n\${cfLines.join('\\n')}\${sizing.length?'\\n\\n'+sizing.join('\\n'):''}\`;";
