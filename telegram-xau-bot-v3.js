...

{
  ...
  const newTarget="const labels=Array.isArray(s?.targetLabels)?s.targetLabels:[],targetLines=t.map((v,i)=>valid(v)?\`🎯 TP\${i+1}: \${n(v)}\${labels[i]?' • '+labels[i]:''}\`:null).filter(Boolean);const mom=s?.momentum||s?.prediction||{},sp=s?.scenarioPlan||mom?.scenarioPlan||{},lot=s?.lotSizing||null,activeZone=s?.side==='BUY'?sp?.buy:sp?.sell,trigger=activeZone?.trigger?.type||s?.prediction?.triggerType||'5m',exp=Number(mom?.volatilityExpansion??mom?.expansionRatio??s?.volatilityExpansion);const momentumLines=[\`🧭 Strategy: \${String(s?.strategy||'LARRY_WILLIAMS')}\`,\`📍 15m trend: \${String(s?.contextBias||'—')}\`,Number.isFinite(Number(mom?.williamsR5))?\`📈 Williams %R: \${Number(mom.williamsR5).toFixed(1)}\`:null,Number.isFinite(exp)?\`⚡ Volatility expansion: \${exp.toFixed(2)}×\`:null,lot&&Number(lot.recommendedLot)>0?\`📐 اللوت المحسوب: \${Number(lot.recommendedLot).toFixed(2)} lot • Risk ≈ $\${Number(lot.actualRiskUsd||0).toFixed(2)}\`:null].filter(Boolean);return \`\${icon} XAUUSD — \${side}\\n✅ ZONE SCENARIO CONFIRMED\\n📊 درجة الإعداد: \${confidence}\\n💵 الدخول: \${money(entry)}\\n🛑 SL: \${n(sl)}\\n\${targetLines.join('\\n')}\\n\\n\${momentumLines.join('\\n')}\${sizing.length?'\\n\\n'+sizing.join('\\n'):''}\`;";
