...

{
  ...
  const newTarget="const labels=Array.isArray(s?.targetLabels)?s.targetLabels:[],targetLines=t.map((v,i)=>valid(v)?\`🎯 TP\${i+1}: \${n(v)}\${labels[i]?' • '+labels[i]:''}\`:null).filter(Boolean);const mom=s?.momentum||s?.prediction||{},lot=s?.lotSizing||null,momentumLines=[\`🧭 Strategy: \${String(s?.strategy||'LARRY_WILLIAMS')}\`,\`📍 15m trend: \${String(s?.contextBias||'—')}\`,Number.isFinite(Number(mom?.williamsR5))?\`📈 Williams %R: \${Number(mom.williamsR5).toFixed(1)}\`:null,Number.isFinite(Number(mom?.volatilityExpansion))?\`⚡ Volatility expansion: \${Number(mom.volatilityExpansion).toFixed(2)}×\`:null,lot&&Number(lot.recommendedLot)>0?\`📐 اللوت المحسوب: \${Number(lot.recommendedLot).toFixed(2)} lot • Risk ≈ $\${Number(lot.actualRiskUsd||0).toFixed(2)}\`:null].filter(Boolean);return \`\${icon} XAUUSD — \${side}\\n✅ LARRY WILLIAMS CONFIRMED\\n📊 درجة الإعداد: \${confidence}\\n💵 الدخول: \${money(entry)}\\n🛑 SL: \${n(sl)}\\n\${targetLines.join('\\n')}\\n\\n\${momentumLines.join('\\n')}\${sizing.length?'\\n\\n'+sizing.join('\\n'):''}\`;";
