...
function patch(source){
  // Keep BTC automatic execution at a minimum of 65% confidence.
  source=source.replaceAll("base.confidence>=(mtfConfirm>=2&&candidate===mtfSide?52:m1Override&&candidate===microSide?50:58)","base.confidence>=65");
  source=source.replaceAll("candidate===active.side&&base.confidence>=(mtfConfirm>=2&&candidate===mtfSide?52:m1Override&&candidate===microSide?50:58)","candidate===active.side&&base.confidence>=65");
  source=source.replaceAll("base.confidence>=58&&btcAutoTradeState.history.filter(x=>now-Number(x.issuedAtMs||0)<24*60*60_000).length<30","base.confidence>=65&&btcAutoTradeState.history.filter(x=>now-Number(x.issuedAtMs||0)<24*60*60_000).length<30");
  source=source.replaceAll("base.confidence>=58&&Math.abs(price-active.entry)>repriceThreshold","base.confidence>=65&&Math.abs(price-active.entry)>repriceThreshold");
