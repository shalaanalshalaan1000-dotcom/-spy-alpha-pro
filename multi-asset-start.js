...
async function getBtcAutoTradeSignal(execute=false){
  ...
  const fastMomentum=Boolean(c.rsiMomentum||c.structureMomentum||c.hourTrendConfirm||c.impulseConfirm),againstHtf=Boolean(c.againstHtf);
  const fastEligible=hardCandidate==='WAIT'&&fastDirection!=='WAIT'&&fastMomentum&&!againstHtf&&Number(raw.confidence||0)>=66;
  const candidate=hardCandidate!=='WAIT'?hardCandidate:fastEligible?(fastDirection==='UP'?'BUY':'SELL'):'WAIT';
  const rawEntry=Number(raw.entry),direction=candidate==='BUY'?1:candidate==='SELL'?-1:0,scalpDistance=Number.isFinite(price)&&price>0?Math.min(70,Math.max(22,price*0.00035)):null,scalpStopDistance=Number.isFinite(scalpDistance)?Math.max(18,scalpDistance*0.8):null,scalpEntry=Number.isFinite(rawEntry)&&rawEntry>0?rawEntry:price,scalpTarget1=direction&&Number.isFinite(scalpDistance)?scalpEntry+direction*scalpDistance:Number(raw.target1),scalpTarget2=direction&&Number.isFinite(scalpDistance)?scalpEntry+direction*scalpDistance*1.6:Number(raw.target2),scalpTarget3=direction&&Number.isFinite(scalpDistance)?scalpEntry+direction*scalpDistance*2.2:Number(raw.target3),scalpTarget4=direction&&Number.isFinite(scalpDistance)?scalpEntry+direction*scalpDistance*3:Number(raw.target4),rawStop=Number(raw.invalidation),scalpStop=direction&&Number.isFinite(scalpStopDistance)?scalpEntry-direction*scalpStopDistance:rawStop,entryHalf=Number.isFinite(scalpDistance)?Math.min(16,Math.max(7,scalpDistance*0.32)):null;
  let confidence=Number(raw.confidence||0);if(fastEligible){confidence=Math.max(confidence,66)+(c.impulseConfirm?5:0)+(c.structureMomentum?4:0)+(c.rsiMomentum?3:0)+(c.hourTrendConfirm?3:0);confidence=Math.min(90,confidence)}
  const mode=fastEligible?'BTC_MTF_FAST_SCALP':String(raw.signalMode||'BTC_ICT_MOMENTUM')+'_SCALP';
  const base={asset:'BTCUSD',action:'WAIT',candidateAction:candidate,status:candidate==='WAIT'?'WAIT':'CANDIDATE',strategy:mode,confidence:autoRound(confidence,0),price:autoRound(price),entry:autoRound(scalpEntry),entryLow:autoRound(Number.isFinite(entryHalf)?scalpEntry-entryHalf:raw.entryRangeLow),entryHigh:autoRound(Number.isFinite(entryHalf)?scalpEntry+entryHalf:raw.entryRangeHigh),stopLoss:autoRound(scalpStop),target1:autoRound(scalpTarget1),target2:autoRound(scalpTarget2),target3:autoRound(scalpTarget3),target4:autoRound(scalpTarget4),riskReward:Number.isFinite(scalpTarget1)&&Number.isFinite(scalpStop)&&Math.abs(scalpEntry-scalpStop)>0?autoRound(Math.abs(scalpTarget1-scalpEntry)/Math.abs(scalpEntry-scalpStop),2):null,updatedAt:new Date(now).toISOString(),provider:String(raw.provider||'COINBASE'),reason:candidate==='WAIT'?'انتظار اتجاه 15د مع تأكيد زخم 5د/1د':fastEligible?'BTC سريع: اتجاه 15د + زخم/بنية 5د و1د؛ لا يشترط اكتمال ICT':String(c.setup||'BTC scalp جاهز؛ الهدف الأول قريب لخروج سريع')};
  ...
  if(active){
    const stopped=active.side==='BUY'?price<=active.stopLoss:price>=active.stopLoss,completed=autoTradeTargetReached(active,price,active.target1),opposite=['BUY','SELL'].includes(candidate)&&candidate!==active.side&&base.confidence>=68,expired=now-active.issuedAtMs>10*60_000;
    if(stopped||completed||opposite||expired){btcAutoTradeState.signal=null;btcAutoTradeState.cooldownUntil=now+3*60_000;active=null}
  }
  if(!active&&execute&&now>=btcAutoTradeState.cooldownUntil&&['BUY','SELL'].includes(candidate)&&base.confidence>=66){
...
`;
  ...
  for (const marker of ['function getBtcAutoTradeSignal', "asset.startsWith('BTC')", 'BTC_MTF_FAST_SCALP', 'MT5_USER_CONTROLLED']) {
