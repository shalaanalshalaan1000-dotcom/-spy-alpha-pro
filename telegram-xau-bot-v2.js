...
let ready=false;
const sent={side:null,key:null,above:false,messageId:null,lastText:null,lastEditMs:0};
...
async function tick(){
  try{
    ...
    if(terminalMatchesLock(tradeLock,s)){
      console.log(`[telegram-xau-confirmed] trade lock released ${tradeLock.side} key=${tradeLock.key} outcome=${s.terminalEvent?.outcome||s.terminalEvent?.result||'CLOSED'}`);
      resetSent();
      clearTradeLock();
    }
...
console.log(`[telegram-xau-confirmed] ${BOT_TOKEN&&CHAT_ID?'enabled':'disabled'} one-active-trade lock; threshold=${SIGNAL_THRESHOLD}; 5m-close=${CONFIRM_ON_5M_CLOSE}; no-candidate/no-late mode`);
if(process.env.NODE_ENV!=='test')(async function loop(){while(true){await tick();await new Promise(r=>setTimeout(r,POLL_MS));}})();

export {targetMessage,canSendSignal,fiveMinuteCloseConfirmed,terminalMatchesLock,lockAllowsSignal,signalKey};
